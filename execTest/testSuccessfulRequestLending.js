...
module.exports = function(callback) {
    ...
    return JJCoinContract.new().then(function (instance) {
        ...
        console.info("Successful request lending");
        console.info("============================================================");
        ...
        console.info("Step 4: borrower paybacks the loan");
        return jjCoin.approve(jjLend.address, paybackJJCoin, {from: borrower});
    }).then(function () {
        return jjCoin.allowanceOf(borrower, jjLend.address);
    }).then(function (val) {
        assert(paybackJJCoin, val);
        console.info("borrower allows " + val.toNumber() + " JJCoin to JJLend");
        return jjLend.paybackLoan(borrowerRequestID, {from: borrower});
    }).then(function () {
        console.info("borrower successfully paid back loan!")
        return jjCoin.balanceOf(borrower, {from: borrower});
    }).then(function (val) {
        console.info("borrower JJCoin balance: " + val.toNumber());
        return jjCoin.balanceOf(lender, {from: lender});
        ...
        assert.equal(borrower, val);
        console.info("collateral[" + colID + "] returned to borrower: " + (val == borrower));
        return jjLend.getCredit(borrower);
    }).then(function (val) {
        assert.equal(borrowJJCoin / 1000, val.toNumber());
